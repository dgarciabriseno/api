...
class HVSTEREODataServer(DataServer):
    def __init__(self):
        ...
        self.euvi_wavelengths = [171, 195, 284, 304]
        self.cor_detectors = ["1-A", "1-B", "2-A", "2-B"]
    ...
    def compute_groups(self):
        ...
        groups = []
        # EUVI
        for meas in self.euvi_wavelengths:
            groups.append(','.join(["helioviewer","EUVI-A", str(meas)]))
            groups.append(','.join(["helioviewer","EUVI-B", str(meas)]))
        # COR
        for detector in self.cor_detectors:
            groups.append(','.join(["helioviewer","COR" + detector, "white-light"]))
        return groups
