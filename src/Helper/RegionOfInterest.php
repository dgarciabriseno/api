...
class Helper_RegionOfInterest
{
    ...
    /**
     * Returns a new region of interest which has been rotated by the given angle
     * with the axis of rotation being sun center.
     *
     * Note that the region of interest is always a rectangle without any rotation.
     * This rotation operation expands the region such that the new rectangle
     * is just big enough to contain the rotated original rectangle.
     */
    public function rotate(float $degrees): Helper_RegionOfInterest {
        // Rotate each corner of the region around sun center (0, 0)
        $upper_left = new Point($this->_left, $this->_top);
        $upper_left->rotate($degrees);
        $upper_right = new Point($this->_right, $this->_top);
        $upper_right->rotate($degrees);
        $bottom_left = new Point($this->_left, $this->_bottom);
        $bottom_left->rotate($degrees);
        $bottom_right = new Point($this->_right, $this->_bottom);
        $bottom_right->rotate($degrees);

        // Assign the new bounds to left, top, right, and bottom.
        $left = min($upper_left->x, $upper_right->x, $bottom_left->x, $bottom_right->x);
        $right = max($upper_left->x, $upper_right->x, $bottom_left->x, $bottom_right->x);
        $top = min($upper_left->y, $upper_right->y, $bottom_left->y, $bottom_right->y);
        $bottom = max($upper_left->y, $upper_right->y, $bottom_left->y, $bottom_right->y);

        return new Helper_RegionOfInterest($left, $top, $right, $bottom, $this->_scale, $this->_maxWidth, $this->_maxHeight);
    }
    ...
    /**
     * Sets the region of interest width in arcseconds by stretching to the right.
     *
    ...
    /**
     * Sets the region of interest height by stretching downwards
     */
